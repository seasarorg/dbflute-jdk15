...
public abstract class BsMemberStatusBhv extends AbstractBehaviorWritable {
    ...
    protected <ENTITY extends MemberStatus> ENTITY doSelectEntity(final MemberStatusCB cb, final Class<ENTITY> entityType) {
        assertCBStateValid(cb);
        return helpSelectEntityInternally(cb, new InternalSelectEntityCallback<ENTITY, MemberStatusCB>() {
            public List<ENTITY> callbackSelectList(MemberStatusCB cb) { return doSelectList(cb, entityType); } });
    }
    ...
    protected <ENTITY extends MemberStatus> ENTITY doSelectEntityWithDeletedCheck(final MemberStatusCB cb, final Class<ENTITY> entityType) {
        assertCBStateValid(cb);
        return helpSelectEntityWithDeletedCheckInternally(cb, new InternalSelectEntityWithDeletedCheckCallback<ENTITY, MemberStatusCB>() {
            public List<ENTITY> callbackSelectList(MemberStatusCB cb) { return doSelectList(cb, entityType); } });
    }
    ...
    protected int doQueryUpdate(MemberStatus memberStatus, MemberStatusCB cb, UpdateOption<MemberStatusCB> option) {
        ...
        prepareUpdateOption(option);
        return delegateQueryUpdate(memberStatus, cb, option);
    }
